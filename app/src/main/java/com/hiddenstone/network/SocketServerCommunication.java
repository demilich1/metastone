...
import net.demilich.metastone.MetaStoneSimpleServer;
				...
				public void run() {
					try {
						...
						while(shouldRun){
							...
							System.out.println(message.mt.toString());
							if (message.mt == MessageType.registerPlayer){
								listener.onPlayerConnected(message.player1, message.player2);
							} else {
								listener.onActionRegistered(message.callingPlayer, message.action);;
							}
						}
